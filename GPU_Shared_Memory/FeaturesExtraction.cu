 ...
 * color histograms, texture histograms, oriented gradient histograms, and Hu moments using shared memory.
...
// Kernels declaration
__global__ void calculateRGBMeansKernelShared(unsigned char* patch, float* rgbMeans, int totalPixels);
__global__ void calculateColorHistogramKernelShared(unsigned char* patch, float* histogram, int totalPixels);
__global__ void calculateOrientedGradientHistogramKernelShared(unsigned char* patch, float* histogram, int totalPixels, int patchWidth, int patchHeight, int numChannels);
__global__ void calculateTextureHistogramKernelShared(unsigned char* patch, float* histogram, int totalPixels, int patchWidth, int patchHeight, int numChannels);
__global__ void calculateHuMomentsKernelShared(unsigned char* patch, float* huMoments, int patchWidth, int patchHeight, int numChannels);

// ==================== extractFeatures ====================

void extractFeaturesShared(unsigned char* refPatch, unsigned char* posPatch, unsigned char* negPatches, float* embeddings, int numPatches, int patchWidth, int patchHeight, int numChannels) {
    ...
    // Allocate and transfer all patches in one time
    ...
    // Create et initialize CUDA streams
    for (int i = 0; i < numPatches; i++) {
        CHECK_CUDA_ERROR(cudaStreamCreate(&streams[i]));
    }

    // Define blocs and grids sizes
    dim3 blockSize(16, 16);
    dim3 gridSize((patchWidth + blockSize.x - 1) / blockSize.x, (patchHeight + blockSize.y - 1) / blockSize.y);
    size_t sharedMemorySize = patchWidth * patchHeight * numChannels * sizeof(unsigned char); // Calcul de la taille de la mémoire partagée nécessaire
    ...
    for (int p = 0; p < numPatches; p++) {
        ...
        calculateRGBMeansKernelShared<<<gridSize, blockSize, sharedMemorySize, streams[p]>>>(currentPatch, currentEmbeddings, totalPixels);
        calculateColorHistogramKernelShared<<<gridSize, blockSize, sharedMemorySize, streams[p]>>>(currentPatch, currentEmbeddings + 3, totalPixels);
        calculateOrientedGradientHistogramKernelShared<<<gridSize, blockSize, sharedMemorySize, streams[p]>>>(currentPatch, currentEmbeddings + 3 + CH_BINS * CHANNELS, totalPixels, patchWidth, patchHeight, numChannels);
        calculateTextureHistogramKernelShared<<<gridSize, blockSize, sharedMemorySize, streams[p]>>>(currentPatch, currentEmbeddings + 3 + CH_BINS * CHANNELS + GH_BINS, totalPixels, patchWidth, patchHeight, numChannels);
        calculateHuMomentsKernelShared<<<gridSize, blockSize, sharedMemorySize, streams[p]>>>(currentPatch, currentEmbeddings + 3 + CH_BINS * CHANNELS + GH_BINS + TH_BINS, patchWidth, patchHeight, numChannels);
    }
...
__global__ void calculateRGBMeansKernelShared(unsigned char* patch, float* rgbMeans, int totalPixels) {
    extern __shared__ unsigned char sharedPatch[];
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    if (index < totalPixels * CHANNELS) {
        sharedPatch[index] = patch[index];
    }
    __syncthreads();
    ...
    for (int i = index; i < totalPixels; i += stride) {
        ...
        redSum += sharedPatch[pixelIndex];
        greenSum += sharedPatch[pixelIndex + 1];
        blueSum += sharedPatch[pixelIndex + 2];
    }
...
__global__ void calculateColorHistogramKernelShared(unsigned char* patch, float* histogram, int totalPixels) {
    ...
    for (int i = index; i < totalPixels; i += stride) {
        int pixelIndex = i * CHANNELS;
        atomicAdd(&histogram[sharedPatch[pixelIndex]], 1);
        atomicAdd(&histogram[256 + sharedPatch[pixelIndex + 1]], 1);
        atomicAdd(&histogram[512 + sharedPatch[pixelIndex + 2]], 1);
    }
...
__global__ void calculateOrientedGradientHistogramKernelShared(unsigned char* patch, float* histogram, int totalPixels, int patchWidth, int patchHeight, int numChannels) {
    ...
    if (x < patchWidth && y < patchHeight) {
        sharedPatch[index] = patch[index];  // Load pixel data into shared memory
    }
    __syncthreads();

    if (x > 0 && x < patchWidth - 1 && y > 0 && y < patchHeight - 1) {
        int pixelIndex = (y * patchWidth + x) * numChannels;
        float Gx = -1 * sharedPatch[pixelIndex - numChannels - patchWidth*numChannels]
                   + 1 * sharedPatch[pixelIndex + numChannels - patchWidth*numChannels]
                   - 2 * sharedPatch[pixelIndex - numChannels]
                   + 2 * sharedPatch[pixelIndex + numChannels]
                   - 1 * sharedPatch[pixelIndex - numChannels + patchWidth*numChannels]
                   + 1 * sharedPatch[pixelIndex + numChannels + patchWidth*numChannels];
        float Gy = -1 * sharedPatch[pixelIndex - patchWidth*numChannels - numChannels]
                   - 2 * sharedPatch[pixelIndex - patchWidth*numChannels]
                   - 1 * sharedPatch[pixelIndex - patchWidth*numChannels + numChannels]
                   + 1 * sharedPatch[pixelIndex + patchWidth*numChannels - numChannels]
                   + 2 * sharedPatch[pixelIndex + patchWidth*numChannels]
                   + 1 * sharedPatch[pixelIndex + patchWidth*numChannels + numChannels];
...
__global__ void calculateTextureHistogramKernelShared(unsigned char* patch, float* histogram, int totalPixels, int patchWidth, int patchHeight, int numChannels) {
    ...
    if (x > 0 && x < patchWidth - 1 && y > 0 && y < patchHeight - 1) {
        unsigned char center = sharedPatch[(y * patchWidth + x) * numChannels];
        unsigned char code = 0;
        int dx[8] = {-1, 0, 1, 1, 1, 0, -1, -1};
        int dy[8] = {-1, -1, -1, 0, 1, 1, 1, 0};
        for (int i = 0; i < 8; i++) {
            int nx = x + dx[i], ny = y + dy[i];
            unsigned char neighbor = sharedPatch[(ny * patchWidth + nx) * numChannels];
...
__global__ void calculateHuMomentsKernelShared(unsigned char* patch, float* huMoments, int patchWidth, int patchHeight, int numChannels) {
    ...
    if (x < patchWidth && y < patchHeight) {
        ...
        if (numChannels == 1) {
            grayValue = sharedPatch[index];  // Already grayscale
        } else {
            // Convert to grayscale using standard luminosity formula
            grayValue = 0.299f * sharedPatch[index * numChannels] + 0.587f * sharedPatch[index * numChannels + 1] + 0.114f * sharedPatch[index * numChannels + 2];
        }
